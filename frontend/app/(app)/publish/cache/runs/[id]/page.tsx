...
import {
  cancelDomainCacheRun,
  getDomainCacheRun,
  retryFailedDomainCacheRun,
  type CacheAction,
  type DomainCacheRunDetail,
} from "@/lib/domainCache";
...
export default function CacheRunDetailPage() {
  const params = useParams<{ id: string }>();
  const router = useRouter();
  const { t } = useT();
  const id = Number(params.id);

  const [run, setRun] = useState<DomainCacheRunDetail | null>(null);
  ...
  const refresh = useCallback(async () => {
    try {
      setRun(await getDomainCacheRun(id, page, pageSize));
      setLoadError(null);
    } catch (err) {
  ...
  return (
    <main className="mx-auto max-w-6xl px-5 py-6">
      <button
        onClick={() => router.push("/publish/cache/runs")}
      ...
      <div className="mb-4 flex items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-0.5 text-sm text-neutral-500 dark:text-neutral-400">
            <b>{t(ACTION_LABEL[run.action])}</b>
            {run.skipped_unsupported > 0 && (
              <>
                {" · "}
                <span className="text-amber-700 dark:text-amber-400">
                  {t("cacheRun.unsupportedSkipped", { count: run.skipped_unsupported })}
                </span>
              </>
            )}
          </p>
        ...
        <div className="flex shrink-0 items-center gap-2">
          {canCancel && (
            <button
              onClick={() => action(cancelDomainCacheRun)}
              disabled={busy}
          ...
          {canRetry && (
            <button
              onClick={() => action(retryFailedDomainCacheRun)}
              disabled={busy}
              title={t("cacheRun.retryFailedHint", { count: run.failed })}
            ...
            >
              {t("cacheRun.retryFailed", { count: run.failed })}
            </button>
          )}
        </div>
      </div>
      ...
      <div className="mb-4 rounded-lg border border-neutral-200 bg-white p-4 dark:border-neutral-800 dark:bg-neutral-900">
        <div className="mb-2 flex items-center justify-between text-xs text-neutral-700 dark:text-neutral-300">
          <span>{t("cacheRun.processed", { done: processed, total: run.total })}</span>
          <span>{pct}%</span>
        </div>
        ...
        <div className="mt-2 flex flex-wrap gap-3 text-xs text-neutral-500 dark:text-neutral-400">
          <span>{t("cacheRun.donePrefix", { count: run.done })}</span>
          {run.failed > 0 && (
            <span className="text-red-600 dark:text-red-400">
              {t("cacheRun.failedPrefix", { count: run.failed })}
            </span>
          )}
          {run.skipped > 0 && <span>{t("cacheRun.skippedPrefix", { count: run.skipped })}</span>}
        </div>
      </div>
        ...
        <table className="min-w-full divide-y divide-neutral-200 text-sm dark:divide-neutral-800">
          <thead className="bg-neutral-50 dark:bg-neutral-900/50">
            <tr className="text-left text-xs font-medium uppercase tracking-wide text-neutral-500 dark:text-neutral-400">
              <th className="px-3 py-2">{t("cacheRun.colDomain")}</th>
              <th className="px-3 py-2">{t("cacheRun.colBaseUrl")}</th>
              <th className="px-3 py-2">{t("cacheRun.colStatus")}</th>
              <th className="px-3 py-2">{t("cacheRun.colDetail")}</th>
              <th className="px-3 py-2">{t("cacheRun.colTime")}</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-neutral-200 dark:divide-neutral-800">
            ...
            {run.items.map((it) => (
              <tr key={it.id}>
                ...
                <td className="px-3 py-2 font-mono text-xs text-neutral-600 dark:text-neutral-400 break-all">
                  {it.base_url}
                </td>
                <td className="whitespace-nowrap px-3 py-2">
                  ...
                  {it.clear_status_code != null && (
                    <span className="ml-1 inline-flex items-center rounded bg-neutral-100 px-1 py-0.5 font-mono text-[10px] font-medium text-neutral-600 ring-1 ring-inset ring-neutral-300 dark:bg-neutral-800 dark:text-neutral-400 dark:ring-neutral-700">
                      {t("cacheRun.clearCode", { code: it.clear_status_code })}
                    </span>
                  )}
                  {it.warm_status_code != null && (
                    <span className="ml-1 inline-flex items-center rounded bg-neutral-100 px-1 py-0.5 font-mono text-[10px] font-medium text-neutral-600 ring-1 ring-inset ring-neutral-300 dark:bg-neutral-800 dark:text-neutral-400 dark:ring-neutral-700">
                      {t("cacheRun.warmCode", { code: it.warm_status_code })}
                    </span>
                  )}
                </td>
                <td
