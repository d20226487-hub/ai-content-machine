...
export default function CacheRunsPage() {
  ...
  const [data, setData] = useState<DomainCacheRunsPage | null>(null);
  const [error, setError] = useState<string | null>(null);

  const refresh = useCallback(async () => {
    try {
      setData(await listDomainCacheRuns(page, 20));
      setError(null);
    } catch (e) {
            ...
            <table className="min-w-full divide-y divide-neutral-200 text-sm dark:divide-neutral-800">
              <thead className="bg-neutral-50 dark:bg-neutral-900/50">
                <tr className="text-left text-xs font-medium uppercase tracking-wide text-neutral-500 dark:text-neutral-400">
                  <th className="px-3 py-2">{t("cacheRuns.colRun")}</th>
                  <th className="px-3 py-2">{t("cacheRuns.colAction")}</th>
                  <th className="px-3 py-2">{t("cacheRuns.colStatus")}</th>
                  <th className="px-3 py-2">{t("cacheRuns.colProgress")}</th>
                  <th className="px-3 py-2">{t("cacheRuns.colCreated")}</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-neutral-200 dark:divide-neutral-800">
                {data.items.map((r) => {
                  const processed = r.done + r.failed + r.skipped;
                  ...
                  return (
                    <tr key={r.id} className="hover:bg-neutral-50 dark:hover:bg-neutral-800/40">
                      <td className="px-3 py-2">
                        <Link
                          href={`/publish/cache/runs/${r.id}`}
                          className="font-medium text-blue-600 hover:underline dark:text-blue-400"
                        >
                          {t("cacheRun.runHash", { id: r.id })}
                        </Link>
                      </td>
                          ...
                          <span className="text-xs tabular-nums text-neutral-500 dark:text-neutral-400">
                            ...
                            {r.failed > 0 && (
                              <span className="ml-1 text-red-600 dark:text-red-400">
                                ({t("cacheRun.failedShort", { count: r.failed })})
                              </span>
                            )}
                          </span>
                        </div>
